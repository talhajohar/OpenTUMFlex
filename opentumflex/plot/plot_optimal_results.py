...
def compare_optimal_results(ems,ems2,init_time_step,end_time_step,flex_device,flex_value, prnt_pgr=False, show_balance=True):
    # plot electricity balance
    # ts = ems_local['time_data']['time_slots'].tolist()
    # ems2 is ems_without_flex
    ...
    opt_res2 = ems2['optplan']
    for param in opt_res2:
        opt_res2[param] = np.array(opt_res2[param])
    ...
    if show_balance:
        ...
        ax1.axhline(linewidth=2, color="black")
        idx_plt = (np.linspace(0, N - 1, 5, endpoint=True)).astype(int)
        ax1.axvspan(init_time_step, end_time_step, color="blue", alpha=0.1)
        ...
        ax1.tick_params(axis="y", labelsize=font_size - 2)
        ax1.set_ylabel('Electrical demand [kW]', fontsize=font_size-2)
        ax1.set_title('Electricity balance ('+ flex_device+ ')', fontsize=font_size)
        ax1.set_xticks(ind[idx_plt])
        ax1.set_xticklabels(ts[idx_plt])
        plt.setp(ax1.get_xticklabels(), visible=False)
        # select which legends are to be shown
        ...
        # plt.tight_layout(rect=[0, 0, 1, 1])
        ax1.margins(x=0)
        ax1.text(s ='{0:.2f} kW'.format(flex_value), x= end_time_step+1,y = top-0.5,fontsize = font_size-6,fontweight= 'bold')
        #  plot heat balance
        # plots
        # plt.subplot(2, 1, 2)
        # fig = plt.figure(figsize=(10, 6))
        ...
        p10 = ax2.bar(ind, opt_res['CHP_elec_pow'], width, bottom=opt_res['bat_output_power'],
                color='skyblue', align='edge', alpha = alph)
        p11 = ax2.bar(ind, opt_res['PV_power'], width, bottom=opt_res['bat_output_power'] + opt_res['CHP_elec_pow'],
                color='goldenrod', align='edge', alpha = alph)
        p12 = ax2.bar(ind, opt_res['bat_output_power'], width, color='indianred', align='edge', alpha = alph)
        p13 = ax2.bar(ind, -opt_res['bat_input_power'], width, color='indianred', align='edge', alpha = alph)
        p14 = ax2.bar(ind, opt_res['grid_import'], width,
                bottom=opt_res['bat_output_power'] + opt_res['CHP_elec_pow'] + opt_res['PV_power'],
                color='grey', align='edge', alpha = alph)
        p15 = ax2.bar(ind, -opt_res['grid_export'], width, bottom=-opt_res['bat_input_power'],
                color='darkseagreen', align='edge', alpha = alph)
        p16 = ax2.bar(ind, -opt_res['EV_power'], width, bottom=-opt_res['bat_input_power'] - opt_res['grid_export'],
                color='plum', align='edge', alpha = alph)
        p17 = ax2.bar(ind, -opt_res['HP_elec_power'], width,
                bottom=-opt_res['bat_input_power'] - opt_res['grid_export'] - np.array(opt_res['EV_power']),
                color='wheat', align='edge', alpha = alph)
        p18 = ax2.step(indplus1, np.append(opt_res['Last_elec'], 0), linewidth=2, where='post', color='k', alpha = alph)
        ...
        ax2.tick_params(axis="x", labelsize=font_size - 2)
        ax2.tick_params(axis="y", labelsize=font_size - 2)
        ax2.set_ylabel('Electrical demand [kW]', fontsize=font_size-2)
        ax2.set_title('Electricity balance(No offer)', fontsize=font_size)
        ax2.set_xticks(ind[idx_plt])
        ax2.set_xticklabels(ts[idx_plt])
        plt.setp(ax2.get_xticklabels(), visible=False)
        # select which legends are to be shown
        selector = {np.count_nonzero(opt_res['CHP_elec_pow']): [p10[0], 'CHP'],
                    np.count_nonzero(opt_res['PV_power']): [p11[0], 'PV'],
                    np.count_nonzero(opt_res['bat_output_power'] + opt_res['bat_input_power']): [p12[0], 'Battery'],
                    np.count_nonzero(opt_res['grid_import']): [p14[0], 'Grid_import'],
                    np.count_nonzero(opt_res['grid_export']): [p15[0], 'Grid_export'],
                    np.count_nonzero(opt_res['EV_power']): [p16[0], 'EV'],
                    np.count_nonzero(opt_res['HP_elec_power']): [p17[0], 'HP'],
                    }
        legend_entries = [p18[0]]
        labels_entries = ['Electrical load']
        for count_nonzero, legend_entry in selector.items():
            if count_nonzero > 0:
                legend_entries.append(legend_entry[0])
